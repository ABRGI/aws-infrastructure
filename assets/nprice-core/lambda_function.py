        ...
        for inst in resp['Instances']:
            ...
            if inst['InstanceId'] in instances:
                ...
                if inst['State']['Name'] == "running":
                    ...
                    sns.publish(TopicArn=sns_arn, Message='The {envname} nPrice core EC2 Instance is already running and was for some reason not shut down by the scripts, you might want to check it out.'.format(envname = os.getenv('NPRICE_CORE_INSTANCE_NAME')))
                ...
                # If instance is neither in the started or stopped state, we will send a notification.    
                    ...
                    sns.publish(TopicArn=sns_arn, Message='The {envname} nPrice Core Instance is in an unspecified state (not started or stopped), check it out.'.format(envname = os.getenv('NPRICE_CORE_INSTANCE_NAME')))
