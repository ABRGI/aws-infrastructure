    ...
    constructor(scope: Construct, id: string, props?: cdk.StackProps) {
        ...
        for (const publicSubnet of this.nelsonVPC.publicSubnets) {
            cdk.Aspects.of(publicSubnet).add(
                new cdk.Tag('Name', `${this.nelsonVPC.node.id}-${publicSubnet.node.id.replace(/Subnet[0-9]$/, '')}-${publicSubnet.availabilityZone}`)
            ); 
        }
        ...
        for (const privateSubnet of this.nelsonVPC.privateSubnets) {
            cdk.Aspects.of(privateSubnet).add(
                new cdk.Tag('Name', `${this.nelsonVPC.node.id}-${privateSubnet.node.id.replace(/Subnet[0-9]$/, '')}-${privateSubnet.availabilityZone}`)
            ); 
        }
